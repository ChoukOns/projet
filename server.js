...
app.post('/api/login', async (req, res) => {
    try {
        ...
        res.status(200).send({ mytoken: token, role, etat });
    } catch (error) {
        console.error('Error during login:', error);
        res.status(500).json({
            success: false,
...
app.post('/api/cours', async (req, res) => {
    try {
        ...
        const { titre, fichier, prix, langue } = req.body;

        // Enregistrement du fichier et autres données dans la base de données
        const result = await coursCollection.insertOne({
            id_cours: idCours,
            titre: titre,
            fichier: fichier,
            prix: prix,
        ...
        res.json({ success: true, message: 'Cour ajouté avec succès à la base de données.' });
    } catch (error) {
        console.error('Erreur lors de l\'ajout du fichier :', error);
        res.status(500).json({ success: false, message: 'Une erreur est survenue lors de l\'ajout du fichier.', error: error.message });
    }
