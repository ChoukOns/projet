...
import {faTrash, faEdit, faFileAlt, faPlus, faEye,faFileVideo, faFileAudio, faFile,faBook } from '@fortawesome/free-solid-svg-icons';
...
const handleSubmit = async () => {
  try {
    if (formTitre2  && selectedFile && formPrix && formLangue) {
      const requestData = {
        titre: formTitre2,
...
const handleFileChange2 = (event) => {
  ...
  setSelectedFileName(file ? file.name : ''); // Mettre à jour le nom du fichier seulement s'il est défini
};
            ...
            <Modal show={showQuizModal} onHide={() => setShowQuizModal(false)}>
              ...
              <Modal.Body>
                ...
                <Form.Group controlId="formQuestions">
                  ...
                  <Button variant="primary" onClick={handleAddQuestion}>Ajouter une question</Button>
                </Form.Group>
              </Modal.Body>
              <Modal.Footer>
          ...
          <table className="table table-bordered">
            <thead>
              <tr>
                ...
                <th scope="col">{t('Heure Upload')}</th>
            ...
            <tbody>
              {fichiers.map((fichier, index) => (
                <tr key={fichier.id_fichier}>
                  ...
                  <td>{new Date(fichier.dateUpload).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
                  ...
                  <td>
                    ...
                    <button className="btn-supprimer" onClick={() => deleteFile(fichier)}>
                  ...
                  &nbsp;&nbsp;
                  <button className="btn-transcrire" onClick={() => handleOpenModal3(fichier.textFileName)}>
                      <FontAwesomeIcon icon={faFile} /> 
                  </button>
                </td>
                <td>
                <button className="btn-transcrire" onClick={() => handleOpenModal4()}>
                  <FontAwesomeIcon icon={faFileVideo} /> 
                  </button>
                </td>
                </tr>
...
<Modal show={showModal3} onHide={handleCloseModal3}>
      ...
      <Modal.Body>
        <form>
          ...
          <div className="mb-3">
            <label>Langue:</label>
            <input type="text" value={formLangue} onChange={(e) => setLangue(e.target.value)} />
          </div>
        </form>
      </Modal.Body>
      <Modal.Footer>
        <Button variant="secondary" onClick={() => setShowModal4(false)}>Fermer</Button>
      ...
      {quizzes1?.map((quiz, index) => (
        <div className="grid-item" key={quiz._id}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div>
            <button className="btn-supprimer" onClick={() => handleDeleteQuiz(quiz._id)}> 
                <FontAwesomeIcon icon={faTrash} />
              </button> &nbsp;&nbsp;
